/************************ ADDITIONAL FEATURES ************************
     Plugins
		 ** Cookie Combining Utility: non-standard cookies are used! **
*/
/************************** CONFIG SECTION **************************/
...
var s_linkInternalFilters="javascript:,walmart"
var s_linkLeaveQueryString=false
var s_linkTrackVars="None"
var s_linkTrackEvents="None"
/* MSN Shopping Wrapper Config */
var s_msn_expireAfterDays=0 /* Number of days, 0 for Session */
var s_msn_cookie_name = 's_msn_shop';
var s_wm_ad_ids="0100000015940864802498,0100000015940864602498,0100000015940864702498,0100000015941179405499,0100000015941183605499,1500000000000001557410,1500000000000001363180,1500000000000001363320,1500000000000001363330"
...
function s_doPlugins() {
	...
	var e=s_vp_getValue('s_events')
	var temp=s_c_r('com.wm.customer');
    if(temp){
      temp=temp.substring(5,temp.indexOf('~~'));
      s_vpr('s_prop15',temp);
    }
	if(e && e.indexOf('purchase')>-1){
		var pid=s_vp_getValue('s_purchaseID')
		if(pid)
			s_vpr('s_eVar20',pid)
		else
			s_vpr('s_eVar20','no order id with purchase')
	}
}
...
/* MSN Shopping Wrapper */
function s_msn_wrapper(un){var k=s_gg("msn_cookie_name"),a=s_getV
("msn_expireAfterDays"),l='&msnshopping',e=new Date,q=
s_wd.location.search,s_msn_t=new Array(3),st_eVar1=0,st_campaign=0,
trk=0;s_msn_t[1]='MSN Shopping';s_msn_t[2]='MSN Search';s_msn_t[3]=
'MSN Network';if(un&&k&&(a||a==0)){if(q.substring(0,1)=='?')q=q.substring(1);
q='&'+q;q=q.toLowerCase();var eq=q.indexOf(l)+l.length+1;var v=q.substring
(eq,eq+1);st_campaign=s_getV("s_campaign");var wm_ad_match=false;
var t=s_wm_ad_ids;var c=0;while(t){var i=t.indexOf(',');i=i<0?t.length:i;
t=t.substring(i+1,t.length);c++;}var s_wai=new Array(c);t=s_wm_ad_ids;while
(t){var i=t.indexOf(',');i=i<0?t.length:i;var s=t.substring(0,i);s_wai[c]=s;
t=t.substring(i+1,t.length);c++;};for(var z=0;z<s_wai.length;z++){if
(st_campaign==s_wai[z]){wm_ad_match=true}}if((q.indexOf(l)>=0&&!wm_ad_match)
||(st_campaign&&!wm_ad_match)){s_c_w(k,0,0);}if(q.indexOf(l)>=0&&wm_ad_match)
{e.setTime(e.getTime()+a*86400000);if(a==0)s_c_w(k,1,0);else s_c_w(k,1,e);
s_vpr('s_campaign',s_msn_t[v]);trk=1}else if((s_getV('s_events')).indexOf
('purchase')>=0 && (0|| s_c_r(k)==1)){s_vpr('s_campaign','');trk=1}if(trk)
{st_eVar1=s_getV("s_eVar1");s_vpr("s_eVar1",s_getV("s_purchaseID"));pt=0;
pt2=0;if(s_gg('usePlugins')){s_usePlugins=false;pt=1}if(s_gg('usePlugins'))
{s_usePlugins=false;pt=1}if(s_gg('trackInlineStats')){s_trackInlineStats=
false;pt2=1}s_code=s_dc(un);if(pt)s_usePlugins=true;if(pt2)s_trackInlineStats=
true;s_vpr('s_eVar1',st_eVar1)}if(s_code)s_d.write(s_code);s_vpr('s_campaign'
,st_campaign)}}function s_getV(vs){if(s_getVa(vs)) return s_getVa(vs);else
return'';}function s_getVa(vs){var k=vs.substring(0,2)=='s_'?vs.substring(2):
vs;return s_wd['s_vpm_'+k]?s_wd['s_vpv_'+k]:s_gg(k)}

s_msn_wrapper("devmsnshopwalmart");
