 ...
 * DATE        PERSON       NOTES
 * ----------- -----------  ----------------------------------------------------
 * 27-Jan-2007 Andy Cabezas Added coed to dynamically append MMC parameters to the destination URL
 *
 */
 ...
 */
function cmCreateTechPropsTag(pageID, categoryID, sectionFlag, MMCstring) {
	...
	if (MMCstring)
	{
		MMCstring = 'cm_mmc=' + MMCstring;
		cm.ul = document.location.href + 
				((document.location.href.indexOf("?") < 0) ? "?" : "&") + MMCstring;
	}
...
function cmCreatePageviewTag(pageID, categoryID, searchString, pageNumber, 
               ...
               Signed, WLCCL, WTopic, WTopicR, MMCstring) {
...
function cmCreateProductviewTag(productID, productName, categoryID, quickbuyFlag, 
								sectionFlag, techpropsFlag, pageID, MMCstring) {
 ...
 * returns nothing, causes a document.write of an image request for this tag.
 */
function cmCreateErrorTag(errorType, sectionFlag, MMCstring) {
