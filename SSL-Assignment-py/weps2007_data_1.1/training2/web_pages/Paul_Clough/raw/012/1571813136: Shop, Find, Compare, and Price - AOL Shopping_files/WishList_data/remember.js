function rememberItCountCallback(numItems){
	isgE("rememberItemsDiv").style.height = getElementByAttribute('className', 'rememberItems')[0].offsetHeight + "px";
	isgE('rememberItemCount').innerHTML = "<span style='color: #1289D0; font-weight: bold'>" + numItems.replace("items", "") + "</span> items";
	if(parseInt(numItems) == 0){
		isgE("ddopts").style.display = 'none';
		isgE("allbox").style.display = 'none';
		isgE("allbox").style.visibility	= 'hidden';
		isgE("rememberItemCount").style.visibility = 'visible';
		if(is_aol){
			isgE("rememberItNoData").style.marginTop ='-49px';
		}
		window.onscroll = chkScroll;
		return;	
	}
	else{
		isgE("ddopts").style.display	= 'block';
	}
	isgE("ddopts").style.display = 'block';
	isgE("allbox").style.display = 'block';
	isgE("allbox").style.visibility = 'visible';
	reqWait = 1;
}
...
function adjustRemContainer(){			
	if(getElementByAttribute("className","rememberItems")[0]) if(getElementByAttribute("className","rememberItems")[0].offsetHeight >= '325'){
		isgE("rememberItemsDiv").style.height = '325px';
		isgE("rememberItemsDiv").style.overflow = "auto";
		if(document.all){
			if(isgE('wheredItGoTable')){	
				isgE('wheredItGoTable').style.width = "503px";
			}
		}
		else{
			if(isgE('wheredItGoTable')){isgE('wheredItGoTable').style.width = "501px";}
		}
	}
	else{
		isgE("rememberItemsDiv").style.height = '';
		isgE("rememberItemsDiv").style.overflow = "";
		if(isgE('wheredItGoTable')){isgE('wheredItGoTable').style.width = "100%";}
	}
}
...
function copyIframeContentToDiv(divContent){
	...
	if(dropdownFlag == 1){
		...
		isgE("allbox").style.margin = '0px';
		isgE("allbox").style.padding = '0px 10px 0 0';

		isgE("ri_ddopts").style.marginLeft = '0px';
		isgE("ri_ddopts").style.paddingLeft = '0px';
	}
	else{
...
function toggleAllDrawerItems(formName){
	if(isgE('ri_check_btn')){
		button = isgE('ri_check_btn');
		if(document.forms[formName]){
			var theform = document.forms[formName];
			for(var i = 0; i < theform.elements.length; i++){
				if(theform.elements[i].type == "checkbox"){
					if(button.checked){theform.elements[i].checked = true;}
					else{theform.elements[i].checked = false;}
				}
			}
			if(button.checked){isgE('lbl_ri_check_btn').innerHTML = "Uncheck'em all";}
			else{isgE('lbl_ri_check_btn').innerHTML = "Check'em all";}
		}
	}
}
...
function chkScroll(){
	if(document.documentElement.scrollTop < 100){
		if(getElementByAttribute('className', 'rememberCopy')[0]) getElementByAttribute('className', 'rememberCopy')[0].className = "rememberCopy";
	}
}