/************************ ADDITIONAL FEATURES ************************
     Plugins
*/
/* Altered on: 6/1/2004 by Brian Thaut */
/* Altered on: 12/3/2004 by Brian Olesen: added code for page name with artist and album */
/* Altered on: 5/27/05: G9 code, fixed original referrer
/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here.                       */
/* Variables (s_pageName, s_prop1, etc.) not prefixed with "s_" will not be used. */

/*
This is to deal with the fact we need to set the scOpen event after the first item has been added to the cart.
Cookie gets set to 'true' in mb_global.js when an item is added for the first time.
*/

if(readCookie('1stCartAdd') == 'true' ) {
  
	if(s_events) {
		s_events=s_events+ ",scOpen";
	} else {
		s_events="scOpen";
	}

	setCookie('1stCartAdd','false',null, '/', '.connect.com',false);

}

//this set the buid for omniture tracking
//rporras 062806 getCookie function is deprecated use readCookie instead, it returns a empty string
var userState = readCookie("UserStateCookie");
var buid = "";

if(userState != "" && userState != null){
	var items = userState.split("|");
	buid = items[6];
}
...
var omArtistName;
var omAlbumName;
var ompageName = window.location.host+window.location.pathname;
// Modify the page name to add the artist and album names if they exist - BOlesen

// s_pageName has a limit of 100 char so we have to trim some of the url
try {

	if(ompageName.search('album') > 0 || ompageName.search('artist') > 0){

		var tmpList = ompageName.split("/");
		var start = tmpList[0];
		var middle = tmpList[1];
		var end = tmpList[tmpList.length - 1];
		ompageName = start + "/" + middle + "/" + end;
	}
...
/*
  * rporras 081106 This is redundant now that the new URL convention includes artist and album names. Removed per
  * Peter Bautista Request. 
if (omArtistName != "") {
        suffixTmp = ":artist=" + omArtistName;
}

if (omAlbumName != "") {
		suffixTmp = suffixTmp + ":album=" + omAlbumName;
}

if(omArtistName != "" || omAlbumName != ""){
	s_pageName = ompageName + suffixTmp;
}
*/

try {

	if (s_products.length > 1000){
		endPoint = s_products.indexOf(",",950);
		s_products = s_products.substring(0,endPoint);
	}
...
var s_account=g_omnitureAccount;
/* E-commerce Config */
var s_eVarCFG=""
...
var s_linkDownloadFileTypes="oms,omg,exe,zip,wav,mp3,mov,mpg,avi,doc,pdf,xls"
var s_linkInternalFilters="javascript:,connect,atomz,atom,spde,ciscdb"
var s_linkLeaveQueryString=false
var s_linkTrackVars="s_prop9,s_prop10,s_events,s_eVar2,s_products,s_eVar20,s_prop5"
var s_linkTrackEvents="event1,scAdd,scOpen,scView,scRemove,s_eVar20,event5"
/* Plugin Config */
var s_usePlugins=true
function s_doPlugins() {
	/* Add calls to plugins here */
	// Flash tracking
	s_vp_getCGI('s_prop5','pmid');
	// Double Click and External Campaigns
	s_vp_getCGI('s_campaign','cid');
//	s_vp_getCGI('s_campaign','cdid');
//	s_vp_getMultiCGI('s_campaign','siteid,pageid,adid,creativeid','_',true);
	// Internal campaigns
	s_vp_getCGI('s_eVar1','intcmp');
	// AtomZ Search Terms Click Through
	s_vp_getCGI('s_eVar2','sqry');
	// Coupon Code
	s_vp_getCGI('s_eVar3','promocode');
	// Shopping Cart Type
	s_vp_getCGI('s_eVar4','cart');
	
	var tempVar // temp variable dedicated to holding any referrer if it
	//exists in the query string
	s_vp_getCGI("tempVar","origref")
	if (s_vp_getValue("tempVar") != ""){
		s_vp_getCGI("s_referrer","origref");
	} else {
...
//rporras 062806 Somewhere below the DO NOT ALTER line, it is necessary to set an on the onload of the window.
//currently s_wd.onload is being used, but this overwrites any other onload events so this function addEvent should be used instead.
//the function was renamed to addEventOmniture so it would not conflict with the existing one in c2_global. 
/**
 ...
 */
function addEventOmniture(obj, evType, fn, opt_flag) {
	var flag = false;
	if (opt_flag != null) {
		flag = opt_flag;
	}
...
/**************	  Set Multiple values from CGI ********************/

/*
 * Plugin: Custom Get Multiple Plugin Modified Value
 */
	function s_vp_getMultiCGI(vs,ks,dli,all)
	{var s_fv='';if(ks.indexOf(",")>-1){while(ks.indexOf(",")>-1){t=
	gcgi(ks.substring(0,ks.indexOf(",")));if(t)s_fv=s_fv+dli+
	t;else if(all) return;ks=ks.substring(ks.indexOf(",")+1,ks.length);
	if(ks.indexOf(",")<0){t=gcgi(ks);if(t)s_fv=s_fv+dli+
	gcgi(ks);else if(all) return}}}else s_fv=dli+gcgi(ks);
	var g=s_fv.substring(dli.length,s_fv.length);if(g)s_vpr(vs,g)}function gcgi(k)
...
+'" height=1 width=1 border=0 alt="" style="display:none">');if(!s_d.images[imn])s_ios=0}}}
...
s_wd.onload;addEventOmniture(s_wd,"load",s_wdl,false)}s_wd.s_olr=1}}}function s_iepf(i,a){if(
