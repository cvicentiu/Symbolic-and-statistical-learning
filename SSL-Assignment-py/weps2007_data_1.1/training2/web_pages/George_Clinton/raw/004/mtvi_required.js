if(typeof MTVi=="undefined"){var MTVi=function(){return{util:{},logger:{},config:{},reporting:{},ads:{}};}();}
MTVi.util.Yeti=new function(){
	this.version="2.0";
	this.scriptname="mtvi_required.js";
	this.path="/global/scripts/yeti/";
	this.modules=["core.js","logger.js","util.js","dispatcher.js"];
	MTVi.config.Get={}; // Query string collection for Remote reporting
	MTVi.config.SectionSetup={}; // Overrides collection
	this.appendScript=function(n){
		try{
			if(n.indexOf("/")!=-1){
	...
	this.load=function(){
		try{
			for(var i=0;i<this.modules.length;i++){this.appendScript(this.modules[i]);}
		}catch(e){}
		try{
			for(var i=0;i<MTVi.config.Modules.length;i++){this.appendScript(MTVi.config.Modules[i]);}
		}catch(e){}
	};
	this.getParameters=function(){
		try{
			...
			for(var i in s){
				if(s[i].src!=null){
					if(s[i].src.indexOf(this.scriptname)!=-1){
						var a=s[i].src.split("?")[1].split("&");
						for(var x=0;x<a.length;x++){
							var b=a[x].split("=");
							MTVi.config.Get[b[0]]=b[1].replace(/\W/g,'');
						}
						break;
					}
				}
	...
	this.remote=function(){
		try{
			this.getParameters();
			if(MTVi.config.Get.remote==1){
				this.path="http://www.mtv.com"+this.path;
				if(MTVi.config.Get["channel"].indexOf("neoseeker")>=0 || MTVi.config.Get["channel"].indexOf("bluesnews")>=0) MTVi.config.Get["account"]="viarnd";
			}
			this.load();
		}catch(e){}
	};
	try{if(MTVi.config.path){this.path=MTVi.config.path;}}catch(e){}
	try{if(MTVi.config.Modules){this.load();}else{this.remote();}}catch(e){this.load();}
};

MTVi.reporting.Widget=s_gi('');
MTVi.reporting.Widget.setAttribute=function(k,v){
	try{if(MTVi.util.isDefined(k)){this[k]=MTVi.util.isDefined(v)?v:'';}}catch(e){}
};
MTVi.reporting.Widget.getAttribute=function(k){
	try{if(MTVi.util.isDefined(k)){return this[k];}else{return;}}catch(e){}
};
MTVi.reporting.Widget.setAccount=function(v){
	try{this.setAttribute("un",v);}catch(e){}
};
MTVi.reporting.Widget.getAccount=function(v){
	try{this.getAttribute("un");}catch(e){}
};
