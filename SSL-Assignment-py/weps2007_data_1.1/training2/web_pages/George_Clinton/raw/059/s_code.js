...
s.linkInternalFilters="javascript:,stubhub"
s.linkLeaveQueryString=false
s.linkTrackVars="None"
s.linkTrackEvents="None"
if (window.location.href.indexOf('https') !=0 ) { s.ssl = true; }


/* WARNING: Changing the visitor namespace will cause drastic changes
to how your visitor data is collected.  Changes should only be made
when instructed to do so by your account manager.*/
s.visitorNamespace="stubhub"
...
s.useCommerce=true;
s.varUsed="eVar6";
s.eventList="event1,event2,event3";
...
function s_doPlugins(s) {
	...
	if(!s.prop3)
		s.prop3=s.getQueryParam('event_id');
	if(!s.campaign)
		s.campaign=s.getQueryParam('Affiliate_id');
	if(!s.eVar13)
		s.eVar13=s.getQueryParam('sort_by');
	if(!s.eVar2)
		s.eVar2=s.getQueryParam('g');
	if(!s.eVar12)
		s.eVar12=s.getQueryParam('event_id');
	if(!s.prop7)
		s.prop7=s.getQueryParam('ticket_id');
	if(s.getQueryParam('gSec')=='search')
		s.eVar4="+1";

	if(!s.campaign)
               s.campaign=s.getQueryParam('cmid');
	if(!s.campaign)
	               s.campaign=s.getQueryParam('ticket_finder');
	 
	if(!s.campaign)
	               s.campaign=s.getQueryParam('affiliate_id');
	 
	if(!s.campaign)
	               s.campaign=s.getQueryParam('gcid');
...
/*
 * Capture JavaScript Errors v0.1
 *  Only captures errors that occur after this JS file and before
 *  the call to t(). For best results include the JS file at the
 *  top of the body and call t() at the bottom. Error messages go
 *  into prop25.
 */
window.onerror=new Function("m","e","l",""
 +"var s=s_c_il["+s._in+"];"
 +"s.prop25=l+': '+m;"
);
...
/*
 * Plugin: getQueryParam 1.2 - Return query string parameter values
 */
...
+"'')+'://'+(s.ssl?'was':'wa')+'.stubhub.com/b/ss/'+s.un+'/1/H"
