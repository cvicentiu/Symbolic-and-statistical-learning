...
var s_linkDownloadFileTypes="exe,zip,wav,mp3,mp4,wmv,mov,mpg,avi,doc,pdf,xls"
...
function s_doPlugins() {
	...
	s_vp_getCGI('s_campaign','cmpid');
	s_vp_setValOnce('s_campaign');

	pcName="s_vnum";
	scName="s_sess";
	persistentCookie=s_vp_readCookie(pcName);
	sessionCookie=s_vp_readCookie(scName);
	if(sessionCookie.length>0) {
		if(persistentCookie!="1") 
			s_prop1="Repeat Visitor";
		else
			s_prop1="New Visitor";
	} else {
		if(persistentCookie.length>0) {
			s_vp_setPersistentCookie(pcName,"2");
			s_prop1="Repeat Visitor";
		} else {
			s_vp_setPersistentCookie(pcName,"1");
			s_prop1="New Visitor";
		}
		s_vp_setSessionCookie(scName,"T");
	}
}

function s_vp_setSessionCookie(k,v){var d=s_c_gd();if(k)s_d.cookie=k+
'='+s_ape(v)+'; path=/;'+(d?' domain='+d+';':'');}

function s_vp_setPersistentCookie(k,v){var exp=new Date;var d=s_c_gd();
exp.setYear(((exp.getYear() < 1900) ? 1900 : 0)+exp.getYear()+10);if(k)s_d.cookie=k+
'='+s_ape(v)+'; path=/;'+(d?' domain='+d+';':'')+' expires=' + exp.toGMTString() ;}

function s_vp_readCookie(k) {return s_c_r(k);}
/************************** PLUGINS SECTION *************************/
