...
function dlinit(){
	if(dlini)return;
	dlini=true;
	if (isgE('controlsContainer3')) /*check for PG(PhotoGallery) widget which has controlsContainer3 div*/
	{
		to='dlgo()';
	}
	else{ /*DL widget*/
...
function dlgo(typ,num){
	...
	if(nDls!=1){
		pGet('tab.adp?type=portal&id=dlC&mod=dlDhtml&dlact=dl'+dlnum,dlnum,'dlt','dlC','dlc()');
	}
...
function pggo(typ,num){/*for PG widget*/
	...
	if(nDls!=1){
		pGet('tab.adp?type=portal&id=dlC&mod=dlDhtml&dlact=dl'+dlnum,dlnum,'dlt','dlC','pgc()');
	}
...
function showTab(did,cid,cback){
	...
	if (isgE('controlsContainer3'))/*for PG widget*/
	{
		isgE('controlsContainer3').className='';
	}
	tab.className='di';
...
DM widget JS
****************/
/*show detail function will show details about selected product and hide other product details
...
function Showdetails(idval,wgttype,NoOfProd,wgtId)
{
	...
	switch (wgttype){
		...
		case 'wgt5': 
			isgE('detail_'+idval).style.backgroundColor = '#fff';
			for (var i=1;i<=NoOfProd;i++ ){
				if (isgE('imgdiv_prod'+i)){
					isgE('imgdiv_prod'+i).style.background = 'url(../img/DMwidget/img_divider.gif)';
				}
			}
			var wgtNum = idval.substr(4,5);
			var increcount = (parseInt(wgtNum)+1);
			var decrecount = (parseInt(wgtNum)-1);
			if (isgE('imgdiv_prod'+decrecount)){
				isgE('imgdiv_prod'+decrecount).style.background = 'url(../img/DMwidget/hideImg_divider.gif)';
			}
			if (isgE('imgdiv_prod'+parseInt(wgtNum))){
				isgE('imgdiv_prod'+parseInt(wgtNum)).style.background = 'url(../img/DMwidget/hideImg_divider.gif)';
			}
		break;
	}
	divOn('detail_'+idval);
