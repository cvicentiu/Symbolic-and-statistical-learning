...
function cppAddSelect(select) {
 ...
 var sText=""; 
  if(select.selectedIndex>0 && select.options[select.selectedIndex].value != 'more') {
   ...
   //return "/" + sarr[0] +"-"+sarr[1]+"-" +escape(select.options[select.selectedIndex].text);
  } else {
 sarr=select.name.split(',');
 url ="";
...
function getFilterURL(flag){
 ...
 var url="";
 //url = theform.cid.value +"-"+theform.id.value+"-"+offset+"-"+theform.view.value; 
...
function miniFilterChange(obj) {
 ...
 window.location = window.location+getFilterURL(true)+attrs;
  } else { */
    for (var i = 0; i < theform.elements.length; i++) {
      if (theform.elements[i].type == "select-one") {
        attrs += cppAddSelect(theform.elements[i]);
...
function callPagination(offset){
  ...
  for (var i = 0; i < theform.elements.length; i++) {
   ...
   //var url = theform.cid.value +"-"+theform.id.value+"-"+offset+"-"+theform.view.value;
