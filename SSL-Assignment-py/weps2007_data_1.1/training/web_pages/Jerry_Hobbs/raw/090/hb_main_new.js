//hbx.js,HBX1.4,COPYRIGHT 1997-2005 WEBSIDESTORY,INC. ALL RIGHTS RESERVED. U.S.PATENT No.6,393,479B1 & 6,766,370. INFO:http://websidestory.com/privacy
...
if(location.search&&_TL(_cp)=="null")_cp=_J(location.search,"x");if(_II(document.cookie,"CP=")>-1){
...
function HBXpageChange(page, section){
	//alert("page changed : "+page+" section : "+section);
	_hbPageView(page, "Express/"+section)
}

function HBXchannelChange(section){
	//alert("section changed : "+section);
	_hbPageView("Default", "Express/"+section);
	
}

function HBXexitLink(link){
	//alert("Exiting :"+link);
	_hbExitLink(link);
