...
s.linkInternalFilters="javascript:,powells.com"
...
function s_doPlugins(s) {
	...
	s.prop4=s.getCustomPagePath(s.campaign,"s_p4","Cmp=");
}
...
/* PLUGIN: written by ben hengst to allow for cleaner link tracking */
s.setvar=function(n,v) {this[n]=v;}

s.jtd=function(i,v,t,a) {
   if (a != undefined) {
      var s=s_gi(a);
   }
   else {
...
s.jtd_parse=function () {
   var a=window.jtd_array;
   if(a) {
      //console.info('parsing: '+a);
      for(i=0;i<a.length;i++) {
         try{
            s.jtd(a[i][0],a[i][1],a[i][2]);
         }catch(e){
            console.error(e)
         };
      };
...
if( window.attachEvent ) {
   window.attachEvent( "onload", s.jtd_parse );
} else if( window.addEventListener ) {
   window.addEventListener( "load", s.jtd_parse , false );
}
...
s.visitorNamespace="powellsbooks"
