...
/* E-commerce Config */
var s_eVarCFG="28|-28-28-1-"
/* Link Tracking Config */
var s_trackDownloadLinks=true
var s_trackExternalLinks=true
var s_trackInlineStats=true
var s_linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,doc,pdf,xls,aa"
var s_linkInternalFilters="javascript:,audible"
...
+o.length);i=s.indexOf(o,i+l)}return s}function s_ape(s){return s_rep(
escape(s),'+','%2B')}function s_epa(s){return unescape(s_rep(s,'+',' '
))}function s_pt(s,d,f,a){var t=s,x=0,y,r;while(t){y=t.indexOf(d);y=y<
0?t.length:y;t=t.substring(0,y);r=f(t,a);if(r)return r;x+=y+d.length
t=s.substring(x,s.length);t=x<s.length?t:''}return ''}function s_fl(s,
l){return (s+'').substring(0,l)}var s_c_d='';function s_c_gdf(t,a){if(
...
+'://'+(s_ssl?'102':unc)+'.112.2O7.net/b/ss/'+un+'/'+(s_csss?0:1)+'/G.2-XPd-S/'
...
+'" width=1 height=1 border=0>'}function s_gg(v){var g='s_'+v
...
tm.getTime()/10800000)%10+sed,yr=tm.getYear(),t,q='',qs='';yr=yr<1900?
yr+1900:yr;t=tm.getDate()+'/'+tm.getMonth()+'/'+yr+' '+tm.getHours()
+':'+tm.getMinutes()+':'+tm.getSeconds()+' '+tm.getDay()+' '
+tm.getTimezoneOffset();if(!s_q){s_d.cookie='s_cc=true; path=/';var
tfs=s_gtfs(),tl=tfs.location,r=tfs.document.referrer,s='',c='',v='',
...
s_lnk);if(!o)return '';var p=s_wd.s_g_pageName,w=1,t=s_ot(o),n=s_oid(o
),x=o.s_oidt,h,l,i,oc;if(s_eo&&o==s_eo){while(o&&!n&&t!='BODY'){o=
o.parentElement?o.parentElement:o.parentNode;if(!o)return '';t=s_ot(o)
n=s_oid(o);x=o.s_oidt}oc=o.onclick?o.onclick.toString():'';if(
oc.indexOf("s_gs(")>=0)return ''}h=o.href?o.href:'';i=h.indexOf('?')
h=s_gg('linkLeaveQueryString')||i<0?h:h.substring(0,i);l=s_gg(
'linkName')?s_gg('linkName'):s_ln(h);t=s_gg('linkType')?s_gg(
...
0}p=p?s_fl(p,255):'';if(s_gg('objectID')){n=s_gg('objectID');x=1}t=
s_ot(o);i=o.sourceIndex;if(p&&n&&t)qs='&pid='+s_ape(p)+(w?'&pidt='+w:
'')+'&oid='+s_ape(n)+(x?'&oidt='+x:'')+'&ot='+s_ape(t)+(i?'&oi='+i:'')
}s_wd.linkName=s_wd.s_linkName=s_wd.linkType=s_wd.s_linkType=
s_wd.objectID=s_wd.s_objectID=s_lnk=s_eo=''}if(!trk&&!qs)return '';var
code='';if(un){if(trk&&s_vs(un,sed))code+=s_mr(un,sess,q+(qs?qs:s_rq(
un)));s_sq(un,trk?'':qs)}else if(s_wd.s_unl)
