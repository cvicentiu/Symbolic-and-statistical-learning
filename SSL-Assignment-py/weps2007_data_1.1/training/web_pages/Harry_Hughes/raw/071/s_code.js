...
//var designEnv = domurl.match("design.tii.trb");
//if(designEnv==null){
if(domurl.match("design.tii.trb")){
	s_account="tribpreprod";
}
if(domurl.match("test.tii.trb")){
	s_account="tribpreprod";
}
if(domurl.match("testprod.rc.trb")){
	s_account="tribpreprod";
}
...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,acrobat,mp4,swf,xml,flv,php"
...
function s_doPlugins(s) {
	...
	var tc = "current"		
	if(document.cookie.indexOf('s_path')==-1) {
		s.c_w("s_path",tc,0);
		s.prop30 = "Y"
	}else{
		s.prop30 = "N"
...
/* Set event 5 (page view) on every page - added 7/11/2006 */	

if(s.events){s.events=s.events + ',event5'}else{s.events='event5'} 
...
+"'')+'://'+(s.ssl?'sm':'m')+'.trb.com/b/ss/'+s.un+'/1/H"
