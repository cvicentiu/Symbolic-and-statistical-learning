...
function ImageUploaderWriter(id,width,height){
	...
	this.writeHtml=function(){
		...
		if(isWinIE&&this.activeXControlEnabled){
			...
			document.writeln("<object id=\""+this.id+"\" name=\""+this.id+"\" classid=\"clsid:"+this.activeXClassID+"\" codebase=\""+cb+"\" width=\""+this.width+"\" height=\""+this.height+"\">");
		...
		if(this.javaAppletEnabled){
			...
			if (isWinIE||isSafari){
				if (isWinIE){
					...
					document.writeln("<applet id=\""+this.id+"\" code=\""+this.javaAppletClassName+"\" java_codebase=\"../\" align=\"baseline\" archive=\""+this.javaAppletJarFileName+"\" mayscript=\"true\" scriptable=\"true\" width=\""+this.width+"\" height=\""+this.height+"\">");
				}

				if (this.javaAppletCached&&this.javaAppletVersion!=""){
					this._writeObjectParam("cache_archive",this.javaAppletJarFileName);
				...
				this._writeObjectParam("archive",this.javaAppletJarFileName);
				this._writeObjectParam("code",this.javaAppletClassName);
			...
			//<embed> for all other browsers
				...
				document.write("<embed id=\""+this.id+"\" type=\"application/x-java-applet;version=1.4\" codebase=\""+this.javaAppletCodeBase+"\" code=\""+this.javaAppletClassName+"\" archive=\""+this.javaAppletJarFileName+"\" width=\""+this.width+"\" height=\""+this.height+"\" scriptable=\"true\" mayscript=\"true\" pluginspage=\"http://java.sun.com/products/plugin/index.html#download\"");

				if (this.javaAppletCached&&this.javaAppletVersion!=""){
					this._writeEmbedParam("cache_archive",this.javaAppletJarFileName);
					var v=this.javaAppletVersion.replace(/\,/g,".");
