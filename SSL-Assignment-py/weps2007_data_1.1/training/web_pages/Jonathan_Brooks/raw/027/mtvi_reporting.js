/*Build Date: Wed Jan 24, 2007 13:39:48*/

var com={mtvi:{util:{},logger:{},config:{},ads:{},reporting:{}}};String.prototype.trim=function(){return this.replace(/^\\s+|\\s+$/g,'');}
...
if(typeof document.getElementsByClassName=="undefined"){document.getElementsByClassName=function(s){var nodes=[];var name=new RegExp('\\b'+s+'\\b');var elems=this.getElementsByTagName('*');for(var i=0;i<elems.length;i++){if(name.test(elems[i].className)){nodes.push(elems[i])};}
return nodes;};}
Math.getRnd=function(a_numA,a_numB){if(!com.mtvi.utils.isDefined(a_numA)||isNaN(a_numA)||parseInt(a_numA)<0){a_numA=1;}
if(!com.mtvi.utils.isDefined(a_numB)||isNaN(a_numB)||parseInt(a_numB)<0){a_numB=0;}
if(a_numA<a_numB){var tempNum=a_numA;a_numA=a_numB;a_numB=tempNum;}
return(parseInt(Math.random()*(a_numA-a_numB+1)+a_numB));};com.mtvi.util={isDefined:function(v){if(typeof v==='undefined'||v===null||v===''||v==='undefined'){return false}else{return true};},queryStringToHash:function(str){try{var qs=[];var a=str.split("?")[1].split("&");for(var x=0;x<a.length;x++){var b=a[x].split("=");qs[b[0]]=b[1];}
return qs;}catch(e){}},addOnloadEvent:function(func){try{if(window.attachEvent){window.attachEvent("onload",func);}
if(window.addEventListener){window.addEventListener("load",func,false);}
return this;}catch(e){}},setProperties:function(o){try{for(var i in o.properties){if(typeof o[i]!="object"){var name=i.ucFirst();eval("o.get"+name+"=function(){return this.properties['"+i+"'];}");eval("o.set"+name+"=function(v){this.properties['"+i+"']=this.isDefined(v)?v:'';}");}}}catch(e){try{com.mtvi.logger.JsError(e);}catch(e){}}},debug:function(msg){try{var c=document.getElementById("debug");if(!c){c=document.createElement("div");c.setAttribute("id","debug");document.getElementsByTagName("body")[0].appendChild(c);}
c.innerHTML=c.innerHTML+msg+"<br>";}catch(e){}}};com.mtvi.logger={url:"http://viarnd.112.2o7.net/b/ss/viarnd/1/EMAIL?",send:function(msg){if(MTVi.util.isDefined(msg)){var img=new Image();img.src=msg;}},JsError:function(e){try{var map={message:'c21',name:'c22',stack:'c23'};var msg=this.url+"pageName=JS_ER-"+window.location;if(!MTVi.util.isDefined(e.stack)){e.stack=window.location.href;}
...
+";s.ca()}}",l=window.s_c_il,n=navigator,u=n.userAgent,v=n.appVersion,e=v.indexOf('MSIE '),m=u.indexOf('Netscape6/'),a,i,s;if(l)for(i=0;i<l.length;i++){s=l[i];s.uns();if(s.un==un)return s;else if(s.pt(s.un,',','isf',un)){s=s.co(s);s.un=un;s.uns();return s}}if(e>0){a=parseInt(i=v.substring(e
