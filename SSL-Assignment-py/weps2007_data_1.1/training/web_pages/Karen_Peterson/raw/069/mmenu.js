...
function hideSearch() {if (NS4) {document.layers.searchBox1.visibility = "hidden";document.layers.searchBox2.visibility = "show";}}

function restoreSearch() {if (NS4) {document.layers.searchBox1.visibility = "show";document.layers.searchBox2.visibility = "hidden";}}
...
function popup(mn,mpos){hideSearch(); if(ns4)_d.captureEvents(Event.MOUSEMOVE);_d.onmousemove=getMouseXY;if(isNaN(mn)){for(a=1;a<menus;a++){if(mn==Mname[a])mn=a}}setpos();omv.visibility='hide';clearTimeout(Mtimer);closeallmenus();mtxt='menu'+mn;mi=gmstyle(mtxt);if(mpos){mn=gmobj(mtxt);spos(mn,MouseY+2,MouseX+2,null,null);}flta=0;if((_d.all&&_d.getElementById)&&!mac){ap=gmobj(mtxt);getflta(ap);}sm(mtxt);am=mn}
